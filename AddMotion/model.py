...
class ImageEncoder(Chain):
    def __init__(self,base=64):
        super(ImageEncoder,self).__init__()
        with self.init_scope():
            self.cbr0=CBR_2D(3,base,down=True)
...
class UNet(Chain):
    def __init__(self,in_ch, out_ch=3, base=64):
        super(UNet,self).__init__()
        with self.init_scope():
            self.cbr0=CBR_2D(in_ch,base,down=True)
            self.cbr1=CBR_2D(base,base*2,down=True)
            self.cbr2=CBR_2D(base*2,base*4,down=True)
            self.cbr3=CBR_2D(base*4,base*8,down=True)
            ...
            self.cbr8=L.Convolution2D(base,out_ch,3,1,1,initialW=initializers.Normal(0.01))

    def __call__(self,x,ie=None):
        h1=self.cbr0(x)
        h2=self.cbr1(h1)
        h3=self.cbr2(h2)
        h4=self.cbr3(h3)
...
class Generator(Chain):
    ...
    def __call__(self,x,kpt=None,ie=None):
        h1=self.cbr0(x)
        h2=self.cbr1(h1)
        h3=self.cbr2(h2)
        h4=self.cbr3(h3)
...
class Discriminator_temporal(Chain):
    def __init__(self,base=64):
        super(Discriminator_temporal,self).__init__()
        with self.init_scope():
            self.cbr0=CBR_2D(3 * 4,base,down=True)
            self.cbr1=CBR_2D(base,base)
            self.cbr2=CBR_2D(base,base*2,down=True)
            self.cbr3=CBR_2D(base*2,base*2)
            self.cbr4=CBR_2D(base*2,base*4,down=True)
            self.cbr5=CBR_2D(base*4,base*8,down=True)
