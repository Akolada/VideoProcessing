...
def make_coordinate_grid(img):
    _, _, h, w = img.shape
    x = xp.arange(w).astype(xp.float32)
    y = xp.arange(h).astype(xp.float32)

    x = (2 * (x / (w-1)) - 1)
    y = (2 * (y / (h-1)) - 1)

    xx = xp.tile(x.reshape(1, -1), (h, 1))
    yy = xp.tile(y.reshape(-1, 1), (1, w))

    mesh = chainer.as_variable(xp.concatenate([xx, yy]. axis=2))
...
class KeyPointDetector(Chain):
    ...
    def __call__(self, x):
        heatmap = self.encdec(x)
        final_shape = heatmap.shape
        heatmap = F.softmax(heatmap / 0.1, axis=3)
